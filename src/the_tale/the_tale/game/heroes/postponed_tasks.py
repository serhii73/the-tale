...
class ChooseHeroAbilityTask(PostponedLogic):
    ...
    def process(self, main_task, storage):
        ...
        if self.ability_id not in habilities.ABILITIES:
        ...
        ability_class = habilities.ABILITIES[self.ability_id]

        if not (ability_class.AVAILABILITY.value & habilities.ABILITY_AVAILABILITY.FOR_PLAYERS.value):
